...
// // this script encapsualated in a batch while(true); so it runs again after
// exit.
...
function watch(parse_file_list_dirname, extention, callback) {
	var restart_server = function(filename) {
		...
		if (!ignore) {
			sys.puts((new Date).toTimeString()
					+ ' change discovered, restarting server. the file was: '
					+ filename);
			process.exit();
		} else
			...
			(function() {
				...
				fs.stat(file_on_callback, function(err, stats) {
					...
					if (err) {
						...
						// sys.puts('auto restart - cannot read file1 : '+
						// file_on_callback);
					} else {
						if (stats.isDirectory())
							fs.readdir(file_on_callback, function(err, files) {
								if (err) {
									// do nothing
									// sometimes linked files are missing
									// sys.puts('auto restart - cannot read
									// file2 : '+ file_on_callback);
								} else
									parse_file_list1(file_on_callback, files,
											extention);
							});
						else if (stats.isFile()
								&& file_on_callback.substr(
										file_on_callback.length
												- extention.length)
										.toLowerCase() == extention) // maybe
																		// remove
																		// this
						{
							fs.watchFile(file_on_callback, function() {
								restart_server(file_on_callback);
							}); // probably may consume resources , but also
								// tells whitch file
							// fs.watchFile(file_on_callback, restart_server);
							// //this one consumes less resiurces
						}
					}
				});
			})(); // This creates and executes a new function with its own
					// scope.
		}
	};
	// fs.readdir(parse_file_list_dirname,function(err,files){if (err) throw
	// err;parse_file_list1(parse_file_list_dirname, files,extention);});
	fs.readdir(parse_file_list_dirname, function(err, files) {
		if (err) {
			// do nothing
			// sometimes linked files are missing
			// sys.puts('auto restart -cannot read file3: '+
			// parse_file_list_dirname);
		} else
